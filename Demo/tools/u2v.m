...
% (c) T. Pajdla, pajdla@gmail.com,2017-02-01
function v = u2v(u,Ci)
if nargin>0
    ...
    Cp = C; Cp.k(1) = 0; Cp.r = [0 0 0 0]; % projection by the omni camera without distortion
    up = X2u(X,Cp); 
%     plot3d(uv,'.b');
    uv = u2v(ud,C); % undistorted ray direction vector
    uu = X2u(uv,Cp); % undistorted image coordinates
    plot3d(uv*15,'.m'); %3D ray representative 
    for i = 1:size(uv,2)
        plot3d([[0;0;0] uv(:,i)*15],'c'); %3D ray 
    end
