...
def processPanoramas(panoIds,thisSpaceCutoutsDir,panoramasDir,pano_prefix,fovHorizontal,fovVertical,sensorHeight,sensorWidth):
    ...
    fn = os.path.join(thisSpaceCutoutsDir,'query_mapping.json')
    with open(fn, 'w') as fp:  
...
def cutoutsFromPanorama(panoId,env):
    ...
    for i in range(len(x)):
        ...
        if panoId not in env['Ids_db']:
            ...
            path = os.path.join(thisSpaceQueries, filename)
            plt.imsave(path, panoramaProjection)
...
if __name__ == '__main__':
    ...
    panoramasDir = os.path.join(thisSpaceCutoutsDir, 'rotatedPanoramas',spaceName)
    #meshPath = os.path.join(spaceDir, 'model', '%s.obj' % spaceName)
    meshPath = os.path.join(thisSpaceCutoutsDir, 'model', 'model_rotated.obj' )
    sweepDataPath = os.path.join(thisSpaceCutoutsDir, 'sweepData', '%s.mat' % spaceName)
    debug = True
    

    p, d, files = next(os.walk(panoramasDir))
